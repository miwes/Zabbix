<#
.SYNOPSIS
	Install Zabbix Agent MSI Remote - urceno pro verzi Zabbix agenta v2
.DESCRIPTION
	Version :
...
[CmdletBinding()]Param (
    ...
    ,[Parameter(Mandatory=$False)] [Alias("FileMSI")] [string]$attrFileMSI = '\\server\Zabbix\Agent\zabbix_agent2-6.0.17-windows-amd64-openssl.msi'
    ,[Parameter(Mandatory=$False)] [Alias("ZabbixServer")] [string]$attrZabbixServer = 'x.x.x.x'
    ,[Parameter(Mandatory=$False)] [Alias("ZabbixConfigFiles")] [string]$attrZabbixConfigFiles = '\\pasvmgmt01\Zabbix\Config\'
...
If ($isZabbixAgent) {
        ...
        add-Log -attrText "[Info]; Uninstall Zabbix agent on $attrServername"
        Try {
            ...
            add-Log -attrText "[Error]; Cannot uninstall Zabbix agent on $attrServername; $($Error[0])"
            add-Log -attrText "[Info]; End"
            Exit
        }
